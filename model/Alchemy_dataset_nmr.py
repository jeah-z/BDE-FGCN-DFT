    ...
    def _load(self):
        ...
        def lsmi_to_dgl(smi_id, self_loop=False):
            ...
            if True:
                ...
                g = mol_graph(mol, atomSet, atmId)
                mask = torch.FloatTensor([[int(atmId == i)] for i in atomSet])

                g0 = mol_graph(mol, set0, atmId)
                mask0 = torch.FloatTensor([[int(atmId == i)] for i in set0])
                g1 = mol_graph(mol, set1, atmId)
                mask1 = torch.FloatTensor([[int(atmId == i)] for i in set1])
                g2 = mol_graph(mol, set2, atmId)
                mask2 = torch.FloatTensor([[int(atmId == i)] for i in set2])
                g3 = mol_graph(mol, set3, atmId)
                mask3 = torch.FloatTensor([[int(atmId == i)] for i in set3])
                g4 = mol_graph(mol, set4, atmId)
                mask4 = torch.FloatTensor(
                    [[int(atmId == i)] for i in set4])
                g5 = mol_graph(mol, set5, atmId)
                mask5 = torch.FloatTensor(
                    [[int(atmId == i)] for i in set5])
                g6 = mol_graph(mol, set6, atmId)
                mask6 = torch.FloatTensor(
                    [[int(atmId == i)] for i in set6])
                g7 = mol_graph(mol, set7, atmId)
                mask7 = torch.FloatTensor(
                    [[int(atmId == i)] for i in set7])

                features = feats_cal(mol, atmId)
                features += qm
                # print(len(features))
                # for val/test set, labels are molecule ID
                features = torch.FloatTensor(features)
            ...
            #     print(e)
            #     print(
            #         f'Processing {smi} {atmId} {exp_val}: error encountered!')
            #     return None
        if self.mode == 'Train':
        ...
        for index in self.target.index:
            ...
            #     break
            smi = self.target.loc[index]['SMILES']
            atomId = int(self.target.loc[index]['id'])
            atomType = self.target.loc[index]['type']
            exp_val = self.target.loc[index]['nmr']
            qm = self.target.loc[index]['qm']

            smi_id_list.append([smi, atomId, exp_val, atomType, qm])
