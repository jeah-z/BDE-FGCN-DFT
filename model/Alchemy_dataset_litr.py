...
class TencentAlchemyDataset(Dataset):
    ...
    def _load(self):
        ...
        def mol_graph(mol, set=[], self_loop=False):
            # mol = rdkit.Chem.MolFromSmiles(frag)
            mol = rdkit.Chem.rdmolops.AddHs(mol)
            rdkit.Chem.Kekulize(mol, clearAromaticFlags=True)
            AllChem.Compute2DCoords(mol)
        ...
        def lsmi_to_dgl(smi_id, self_loop=False):
            try:
                ...
                g = mol_graph(mol)
                g0 = mol_graph(mol, set0)
                g1 = mol_graph(mol, set1)
                g2 = mol_graph(mol, set2)
                g3 = mol_graph(mol, set3)
                g4 = mol_graph(mol, set4)
                g5 = mol_graph(mol, set5)
                g6 = mol_graph(mol, set6)
                g7 = mol_graph(mol, set7)
        ...
        for index in self.target.index:
            ...
            #     break
            # smi = self.target.loc[index]['SMILES']
            mol = self.target.loc[index]['mol']
            atoms = mol.GetAtoms()
            atomId = int(self.target.loc[index]['atomId'])
            atomType = atoms[atomId].GetSymbol()
            assert atomType ==self.target.loc[index]['atomType'] 
            print(f'atomType={atomType}, atomId={atomId}')
